...
def get_product_ids(conn):
    ...
    sql_pi = """ select product_id from products as pi 
                where company_code='US001' and product_id <>2  and product_id <>6         
             """
    with conn.cursor() as curs:
...
def so_items(so_id):
    ...
    selling_prices = {1:250000, 3:1500, 4:100, 5:1200}
    shipped_lst = ['yes', 'no']
...
def _to_csv(conn, outfile):
    ...
    with open(os.path.join('data', outfile), 'w') as write_obj:
...
if __name__ == '__main__':
    ...
    _to_csv(conn, outfile='sales_orders_items_10.csv')
    sql_insert = """ insert into sales_orders_items(company_code, sales_order_id, 'product_id', 
                       'units', 'unit_selling_price', 'currency_id', 'tax_code', 'shipped')
                       vaues(%s, %s, %s, %s, %s, %s, %s, %s)
                  """
    #insert_value_tuple(conn, sql_insert)
