...
def get_so_ids_b(conn, start_date, end_date):
    ...
    sql_so_b = """ select so.sales_order_id, cn.customer_id,cn.business_type_id from sales_orders as so
    ...
    #print(len(sales_order_id_b)) #2412
...
def weight_pro():
    ...
    product_weights = [4,0.5,30, 30, 30,5.5] 
...
def so_items_b(so_id_b):
    product_id_1_units=random.choices([1,2,3,4], weights=[0.7, 0.1, 0.1,0.1], k=1)[0]
    product_id_2_units=random.choices([0,1],weights=[0.9,0.1],k=1)[0]
    units_sold_dict = {1:product_id_1_units, 2:product_id_2_units, 3:random.randrange(1, 20),4:random.randrange(1, 100),5:random.randrange(1, 20), 6:random.randrange(10, 300)}
    #print(units_sold_dict)
    selling_prices = dict(get_product_ids(conn))
    shipped_lst = ['yes', 'no']
    shipped_lst_weights = [99,1] # assuming there are 2% of orders were not shipped
...
if __name__ == '__main__':
    ...
    #get_so_ids_b(conn, start_date, end_date)
